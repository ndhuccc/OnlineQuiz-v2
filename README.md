...
| 模式 | 行為 | 適用 |
|------|------|------|
| **純自動** | 啟動後全自動跑：每題開放選項 → 計時 → 收尾 → 下一題 → ... → 開放複習 | 考前模擬、搶分賽 |
| **評量講解** | 老師**每題**手動控制：題幹 → 開放選項 → 收尾 → **學生立即看結果卡**（題幹+選項+你的答案+正解+解析）→ 講解 → 下一題題幹 | 正式評量、課堂觀念釐清 |

模式在「題庫 → Start」時選，**一旦開始測驗後不能切換**。詳細差異見 [docs/USER_MANUAL.md](docs/USER_MANUAL.md#-三-教師課堂控場--投影片互動測驗)。
