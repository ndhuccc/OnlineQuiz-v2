...
開場時（題庫 → Start），老師必須先選一種模式：

| | 純自動模式 | 評量講解模式 |
|---|---|---|
| 徽章顏色 | 灰色 | 琥珀色 |
| 第一題流程 | 啟動後**自動開放選項 + 開始計時** | 啟動後**停在題幹**，老師按「開放選項 & 開始計時」才開始 |
| 每題換題 | 收尾後**自動進下一題**並開放選項 | 收尾後**停在下一題的題幹**，老師再按「開放選項」 |
| 學生交卷後 | 看「請等待」 | **立即看結果卡**：題幹 + 選項（含你選的/正解標記）+ 你的答案 + 正解 + 解析 |
| 講解節奏 | 全自動跑完，老師口頭補充 | 老師**每題**按鈕控制節奏，做完一題講一題 |
| 適用情境 | 考前模擬、搶分賽 | 正式評量、課堂觀念釐清 |

**操作差異只有兩個按鈕**：
- 純自動：**開始測驗**之後系統自己跑，教師只在投影端解說
- 評量講解：每題都要按 **「開放選項 & 開始計時」** 和 **「關閉選項 & 揭曉結果」** 兩個按鈕

```mermaid
graph TD
    A[題庫 → 選模式 → Start] --> B[投影加入頁面與QR Code]
    B --> C[等待學員登入加入]
    C --> D[教師按下開始測驗]
    D --> E{模式?}
    E -- 純自動 --> F1[自動開放第1題選項 + 開始倒數]
    E -- 評量講解 --> F2[停在題幹;老師按'開放選項']
    F1 --> G[作答中]
    F2 --> G
    G --> H[計時到 或 全員交卷]
    H --> I[關閉選項;秀正解+統計]
...
#### 步驟 1：題庫與開場
1. 教師於教師主控台選擇「線性代數」或「程式設計」之題庫。
2. 題庫右側有兩顆 radio：**純自動**（預設）vs **評量講解**。依教學目標選一顆。
3. 按 **Start** 後，投影機畫面上會秀出專屬的測驗邀請卡、加入連結與隨機產生的 6 位數加入代碼。
4. 提示學生至 `http://<教師IP>:5173/` 登入學生帳號後，輸入代碼加入等待。學生若已加入，其姓名或代號會即時浮現在教師的待命投影上。確認全員到齊後，按 **開始測驗 (Start Quiz)**。

> ⚠️ **一旦開始測驗後，模式不能切換**。要重選模式請刪掉這個 session 重新建。

#### 步驟 2：純自動模式作答循環
1. **一鍵開始，自動作答：** 教師按下 **開始測驗** 後，系統立即開放第一題的答案選項並開始倒數計時，學生即可直接作答。
...
4. **自動下一題：** 講評完畢後，系統自動進入下一題並開放選項與倒數計時。最後一題結束後，測驗自動結束。

#### 步驟 2：評量講解模式作答循環
1. **題幹階段：** 啟動後第一題停在題幹（不計時、不顯示選項）。投影文字會提示「題幹階段，準備好請按『開放選項』」。老師可口頭引導學生閱讀題幹。
2. **按「開放選項 & 開始計時」：** 選項即時出現在學生端，計時倒數啟動。
3. **作答中：** 學生提交答案；老師在投影端看「已提交 X/Y 人」。
4. **收尾：** 計時到 或 全員交卷 或 老師手動按「關閉選項」。
5. **結果揭曉（自動）：** 學生端**立即**看到完整結果卡（見下圖），老師端投影顯示正解 + 統計圖。
6. **老師講解：** 依結果講解、補強觀念。
7. **按「下一題」：** 學生端跳到下一題的**題幹**（不自動開選項）。重複步驟 2–7 直至最後一題。
...
#### 步驟 3：釋出複習與觀念定錨（兩種模式都一樣）
測驗結束後，教師在結案控制按鈕下宣佈「釋出本場次成績至複習庫」，學生將能即時在個人帳戶的歷程區完整複讀。
